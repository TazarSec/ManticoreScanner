...
// PollerConfig controls the polling behavior.
...
// PollUntilComplete submits items and polls until all are completed or timeout is reached.
// It returns all results, including any still pending at timeout.
	...
	for {
		...
		if err != nil {
			...
			if errors.As(err, &rlErr) {
				...
				// Extend deadline by the rate-limit wait time.
...
func applyJitter(d time.Duration) time.Duration {
	jitter := 0.8 + rand.Float64()*0.4 // 0.8 to 1.2
...
func sleep(ctx context.Context, d time.Duration) error {
	// Cap sleep to a reasonable maximum.
	if d > 5*time.Minute {
		d = 5 * time.Minute
	}
	t := time.NewTimer(d)
...
// ChunkItems splits items into chunks of the given size.
