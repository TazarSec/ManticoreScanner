...
func TestFormatDetails(t *testing.T) {
	...
	if !strings.Contains(result, "`/bin/sh`") {
		t.Errorf("expected backtick-wrapped /bin/sh, got %q", result)
	}
	...
	// Detail with spaces should not get backticks.
...
func TestEscapeTableCell(t *testing.T) {
	got := escapeTableCell("foo|bar")
	if got != "foo\\|bar" {
		t.Errorf("escapeTableCell = %q, want %q", got, "foo\\|bar")
	}
}