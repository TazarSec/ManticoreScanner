...
// typeAbbreviations maps lowercase tokens to their preferred display form.
...
// humanizeType converts snake_case type names to readable Title Case,
// respecting known abbreviations like DNS, IP, HTTP.
...
func groupFindings(reasons []api.SuspicionReason) []severityGroup {
	...
	for _, r := range reasons {
		...
		m[r.Severity][r.Type] = append(m[r.Severity][r.Type], r.Detail)
	}
...
// formatDetails renders a list of detail strings, deduplicating repeated values
// and truncating when there are more than maxShow unique entries.
func formatDetails(details []string, maxShow int) string {
	...
	for _, e := range unique {
		s := e.detail
		// Use code formatting for short technical values (no spaces).
...
// escapeTableCell escapes pipe characters that would break markdown tables.
...
func writePackageSection(sb *strings.Builder, r api.BatchResultItem) {
	...
	sb.WriteString(fmt.Sprintf("### %s %s `%s`\n", severityIcon(highest), r.Package, r.Version))
	...
	// Findings grouped by severity, then by type.
	...
	for _, sg := range grouped {
		...
		for _, fg := range sg.Groups {
			detail := escapeTableCell(formatDetails(fg.Details, 5))
			sb.WriteString(fmt.Sprintf("| %s | %s |\n", humanizeType(fg.Type), detail))
		}
		sb.WriteString("\n")
	}
